...
def get_detections_from_im(cfg, model, im, image_id, feat_blob_name,
    ...
    with c2_utils.NamedCudaScope(0):
        ...
        #masks = infer_engine.im_detect_mask(model, im_scale, boxes = None)
        #segms = infer_engine.segm_results(model, cls_boxes, masks, None,
...
def main(args):
    ...
    for i, im_name in enumerate(tqdm(im_list)):
        ...
        if im is not None:
            ...
            box_features, max_conf, cls_boxes, objects = \
                    get_detections_from_im(cfg, model, im,
                                           image_id,args.feat_name,
                                           args.min_bboxes,
                                           args.max_bboxes,
                                           bboxes=bbox)

            np.savez(outfile, box_features=box_features, max_conf=max_conf,
                     cls_boxes=cls_boxes, objects = objects)

            if one_giant_file is not None:
                giant_file[im_base_name + '.npz'] = outfile
